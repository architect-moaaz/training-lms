...
import { Users, Building, Shield, Clock, BookOpen, MapPin, Trash2, KeyRound, X, Plus, Library, GraduationCap, BarChart3, Award, Calendar, FolderOpen } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'analytics' | 'users' | 'companies' | 'content' | 'day-content' | 'packages' | 'certificates' | 'events'>('analytics');
  ...
  return (
    <div className="flex-1 px-6 py-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex gap-1 border-b border-white/10 mb-6 overflow-x-auto">
        {(['analytics', 'users', 'companies', 'day-content', 'content', 'packages', 'certificates', 'events'] as const).map((tab) => {
          const icons = { analytics: BarChart3, users: Users, companies: Building, 'day-content': FolderOpen, content: Library, packages: GraduationCap, certificates: Award, events: Calendar };
          const labels = { analytics: 'Analytics', users: 'Users', companies: 'Companies', 'day-content': 'Day Content', content: 'Free Resources', packages: 'Courses', certificates: 'Certificates', events: 'Events' };
